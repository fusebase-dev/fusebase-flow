...
This repo runs **Fusebase Flow v3.19.1**. Read these files before any other action:

- `AGENTS.md` — portable always-on baseline (rules vs skills vs workflows vs hooks vs policies)
- `FLOW_RULES.md` — full always-on rules (FR-01..FR-25) with enforcement-surface map (stop at `## Amendment log` — dated history, never load it)

## Self-attestation (first response of every session)

> "Operating as {Product Owner | AI Developer | Architect (escalation) | Deploy phase} under Fusebase Flow v3.19.1. I will follow FR-01 through FR-25. I will apply Mode A on chat output and Mode B on every internal-artifact write. I will apply the role-discipline skill section for {role}."
