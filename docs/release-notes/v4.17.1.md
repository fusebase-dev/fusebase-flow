# v4.17.1 — command approvals bind the exact operation (BREAKING)

> **Upgrade from 4.16.6 straight to 4.17.1.** `v4.17.0` was tagged and **never published**: its
> release gate went red in a test phase unrelated to this content (`cli-flow-recovery-selectors`
> blew a 180s bound and was stopped by the 1800s phase wall), so no GitHub Release exists for it
> and no consumer can have installed it. The tag stays where it is, immutable, and its fingerprint
> row stands. 4.17.1 carries 4.17.0's content unchanged plus the fix for that phase: it keeps its
> diagnostics instead of deleting them on a timeout, and bounds each case at its own owner.

> **Breaking cutover.** Every command approval artifact you hold stops authorizing on upgrade,
> and each one you still want must be reissued for the operation it authorizes. Nothing is
> migrated, re-dated or reconstructed, and no file is deleted — `--inventory` names each artifact
> that no longer authorizes and why. Protected-path (FR-07) approvals and health-check deferrals
> are unaffected. This is a minor version because no API, CLI or file you author changes and the
> remedy is one documented command; the precedent is `v4.7.0`, which shipped schema v2 with
> mandatory expiry and command-digest binding the same way.
