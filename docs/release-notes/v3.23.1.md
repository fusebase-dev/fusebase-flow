...
`git revert <DEPLOY_HASH>` (and `1e36b8b` if the containment fix itself must come out too) — additive and read-only-safe; a clean revert (no schema/data, no deploy-authority change, no protected-path edit). Tag `v3.23.1`.
