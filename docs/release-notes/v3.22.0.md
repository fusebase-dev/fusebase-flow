...
| **`hooks/local/find-wasted-effort.py`** | Deterministic, stdlib-only, **READ-ONLY** analyzer (563 lines, under the FR-25 800 ceiling). Writes only its own gitignored `state/audit/find-wasted-effort-<date>.md`; NO memory/overlay/spec edits, NO prune/reclassify (D4 — those are Phase 2/3, gated on per-rule FP fixtures). Mirrors token-waste-audit.py's substrate. `--selftest` = 23 synthetic per-rule + `prevents:`-parser fixtures. |
...
`python hooks/local/find-wasted-effort.py` on this repo: 28 artifacts scanned, well-formed report with the FP header, per-rule confirmed/dismissed/inconclusive, and a coverage section listing all 17 governed controls. Rule 6 detected `prevents:` markers in all 6 annotated files. `--selftest` 23/23. No tracked changes beyond the analyzer (report is gitignored).

## Verified

preflight 0/0 · `--selftest` 23/23 · mirrors 0 drift · counts swept 30 → 31 skills / 8 → 9 policies / 5 → 6 commands.
