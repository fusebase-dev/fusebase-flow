...
4. Review the diff after focused checks, before expensive verification. When the scope or the contract itself is undecided, review that premise before writing code. Ordinary maintenance uses implementer review. Changes to ownership, upgrade/recovery authority, security or release publication require an independent review of the relevant contract and diff.
5. Resolve substantive findings together. Before editing, record in the existing outcome record only the unresolved choices and their counterexamples; cover missing evidence, partial failure, mixed outcomes and untrusted input where they apply. Link a correction to its finding instead of restating it, and supersede an instruction the work has overtaken. Re-review the corrections and affected boundaries, expanding only for a concrete new risk. Findings require an input/trigger, wrong outcome and expected behavior. Wording and unrelated improvements do not restart the gate unless they change behavior or a material public claim.
