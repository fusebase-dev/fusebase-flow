...
| **OpenAI / ChatGPT Codex** | `AGENTS.md`, `.codex/config.toml.example`, `.codex/hooks.json.example`, `.agents/skills/<51>/SKILL.md` (32 Flow mirrors + 19 CLI provider skills), `.codex/agents/<4>.md` | Open repo in Codex; copy `config.toml.example` to `config.toml`; accept project trust prompt | Self-attestation, state-announcement footer, Flow skills and CLI provider skills available by skill matching/reference, command parity via the `AGENTS.md` command-equivalents convention (every agent) plus optional `bash hooks/local/install-codex-prompts.sh` for native `/prompts:<cmd>` (per-machine, namespaced, Codex-deprecated), 6 hook events wired via repo-root-stable paths | Project-trust prompt required before hooks load; hook event names not documented as stable contract in v0.1 |
