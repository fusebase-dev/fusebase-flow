...
| `ci-linux-msys-test-divergence` | high | resolved; the pitfall-7 caller ratchet gates the tagged two-platform run from v4.16.2 | Seven cross-platform traps: history depth, shared PATH dirs, file modes, fixture manifests, fixture EOL policy, native CRLF on typed output, and (v4.16.0) the PATH-mask trap re-invented in a new suite where it removed bash itself — a caller ratchet now binds the rule to every caller, not just the fixture |
...
| `docs-only-commit-broke-content-derived-gate` | high | resolved | v4.6.0 tagged with a recorded 619/619 local green, then both CI workflows went red — the green was measured BEFORE the two tail `docs(closeout)` commits, and `9b62819` added a backlog note quoting the attestation string verbatim. `test-sync-allowlist.sh` pruned record doc trees by ENUMERATED name, listing the consumer path `docs/product-backlog` (never existed here) while missing this repo's own `docs/backlog`, so the note read as an unsynced framework file; a second, SPURIOUS failure came from `set -o pipefail` + `missing_set \| grep -q` (grep -q exits at first match ⇒ SIGPIPE ⇒ pipeline 141 ⇒ `!` flips it), a control that had passed only because its output had been exactly one line. NOT platform-specific — failed identically on MSYS and Linux; the "Linux-only" hypothesis came from trusting a stale local green. Fixed: structural `-path './docs/*'` prune (framework doc surface = top-level `docs/*.md` only), pure-bash `has_line()` membership, 2-entry mutation control, and a `docs-surface-is-top-level-only` symmetry guard (v4.6.1) |
| `undecided-contract-drives-repeat-defects` | medium | resolved | v4.7.0's repair ticket took EIGHT review rounds; rounds 2-5 each fixed defects introduced by the round before, because "what does repair confirmed mean when a layer is missing" was never DECIDED — so each call site invented a different answer and every locally-correct patch contradicted the next site. Once decided (M13 bind-at-authorization, M16 anchor outside the repaired tree — M14 was decided on the wrong axis first and refuted, M17 same-principal threat model), rounds 6-8 returned zero class-(b) findings. Trigger: a round whose findings sit inside the previous round's fix. Guardrail: stop implementing, decide the contract |
| `deploy-502-misread-as-logout` | high | resolved | Platform-level: a 502/timeout during `fusebase deploy` (or a scale-to-zero cold start) was read as a logout, falsely signing users out and sometimes clearing the session cookie — a boolean session probe over a three-outcome reality, so "the server said no" and "the server did not answer" shared a branch. Same class: proxy answers `/api/*` with `302` not `401` (default `fetch` follows it into a CORS `TypeError`, no status/body), and `getMe` status `0` is network/CORS, not auth. Fixed upstream in CLI 0.29.8 with a 4-state verdict (`authenticated \| anon \| blocked \| unknown`) + deploy-tolerant retry; Flow was TEACHING the collapsing pattern until the 0.29.8 re-vendor (`3d3dcec`) |
| `readme-version-badge-not-parity-checked` | low | open (backlog) | The README shields.io `version-<X.Y.Z>-blue` badge is the only version surface neither sed'd by `sync-version-strings.sh` nor parity-checked by `preflight.sh` §8 (plugin.json/marketplace are), so a stale badge can ship silently — noted during v4.4.1 (badge corrected by hand). Fix: add a preflight §8 grep-parity check for the badge slug (preferred) or teach sync to rewrite it |
| `cli-overlay-marker-prose-misclassified` | high | resolved | Exact heading/owned-span checks replace product-name prose matching; CUSTOM/FLOW:PRESERVE delimiters are standalone-line structure, so real CLI marker documentation no longer blocks a missing-overlay append (T56). |
