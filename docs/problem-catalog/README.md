...
| `undecided-contract-drives-repeat-defects` | medium | resolved | v4.7.0's repair ticket took SEVEN review rounds; rounds 2-5 each fixed defects introduced by the round before, because "what does repair confirmed mean when a layer is missing" was never DECIDED — so each call site invented a different answer and every locally-correct patch contradicted the next site. Once decided (M13 bind-at-authorization, M16 anchor outside the repaired tree, M17 same-principal threat model), two consecutive reviews returned zero contract findings. Trigger: a round whose findings sit inside the previous round's fix. Guardrail: stop implementing, decide the contract |
