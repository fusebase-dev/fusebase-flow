...
2. **A PATH-dir mask removes git on Linux.** `test-bootstrap-exception.sh` masked `python3` by dropping every PATH dir that contains a python. On `ubuntu-latest` `git` and `python3` **share `/usr/bin`**, so dropping it also removed `git` → the hook's `git diff --cached` found nothing staged → it **exited 0 silently** (the test mis-tested, not a real fail-open). On MSYS git/python live in separate dirs, so the mask kept git → the hook reached its real warn → passed. **Fix: a git-preserving mask (symlink a curated bin excluding only python) + a precondition asserting git survives.**
