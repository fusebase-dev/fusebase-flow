...
## Known residual — the WS3 EXIT-trap reaper does not cover EXTERNAL signals (T3, 2026-08-06)
...
So this entry's HANG fix (tempfile capture) is intact and unaffected; what is open is the ORPHAN case on external teardown. Tracked as `docs/backlog/harness-kill-leaves-orphan-children/`; red arm `hooks/tests/test-run-tests-signal-reap.sh`; evidence `state/audit/run-tests-signal-reap/<full-head>/`.
