...
| `state/` | Runtime state (audit log, approvals, context summary). Git-ignored. Approval artifacts are judged by `hooks/shared/approval_artifact.py` — command approvals need schema 3 (mandatory command + repo binding, the rule's `binding_profile`, and for `git_push_v1` every ref update, re-checked by the `pre-push` hook); protected-path artifacts keep schema 2. `approved_by`/`ticket` are audit metadata, never authenticated authority. `bash hooks/local/approve-local.sh --inventory` names every artifact that no longer authorizes and why. |
