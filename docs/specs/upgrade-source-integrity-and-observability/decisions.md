...
| M13 | The repair-verification contract | Bind the required layer set at authorization; `rc == 0` AND exact `MATCH` per bound layer; empty set refused before any pre-existing file is touched | LOCKED |
...
**Recommendation:** Option (b). `--repair-managed` binds the **required manifest set at authorization time** — the exact claim, narrowed 2026-08-02: **no file that already existed is touched before the bind** — and that set cannot shrink mid-run. Writes may precede the bind, but each only CREATES a new location: the temporary canonical tree under `$TMPDIR` (materialized on every route, removed on exit) and, when the consumer has no staging directory, `.fusebase-flow-source/`. Everything that touches an existing path — `.git/info/exclude`, every managed write — happens after. Binding earlier is impossible: the bind reads the *verified* source, which does not exist until it is materialized. Every layer in the bound set must return **`rc == 0` AND a parsed verdict of exactly `MATCH`**. A layer that is in the bound set but whose manifest or wrapper is absent at verification time is a **failure**, not a skip.
...
| Non-shrinkable | the set is bound before any pre-existing file is touched (M13) and re-read from nothing afterwards |
...
- A consumer genuinely missing a declared layer now **fails** the repair instead of silently narrowing it. This is intended, and recoverable — but **not always by `--repair-managed`**. Repair authorizes only paths a verifier REPORTED: a missing wrapper is hook-layer content and *is* reported, so naming it works; a missing `audit/managed-content-manifest.json` makes its own verifier return `ABSENT` with an empty file list and no other layer covers `audit/`, so naming it is refused as unreported. The diagnostic therefore **derives** the remediation from the live drift report and prints one runnable `RECOVER:` line — `--repair-managed <path>` when the path is reported, otherwise an ordinary upgrade, which reinstalls it as managed content (K13b). A test executes that line and requires the artifact back.
