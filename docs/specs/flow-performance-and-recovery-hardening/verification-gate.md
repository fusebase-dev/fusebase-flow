...
**Status:** BLOCKED at T25 registered-wrapper liveness; T24 focused Git proof rc0 permits its fixture-only commit
**Starting source:** `2217a9c631300e510b18437548ed4bccb5f31036`
**Linked tasks:** `tasks.md` T11-T25
**Review findings:** `adversarial-review.md` B1-B8, N1-N2
**Gate task:** T21, report-only after T25
...
| Registered wrapper liveness | T25 | bounded Windows/MSYS reap regression; incremental engine phase identity; full registered recovery wrapper PASS |
...
| Real Git fixture | `FFCF_T15_ONLY=1 bash hooks/tests/test-cli-flow-recovery.sh` PASS before T24 commit; full wrapper deferred to T25 |
| Timeout/recovery liveness | `bash hooks/tests/test-health-check-timeout.sh`; repeated bounded U17/U18; full `bash hooks/tests/test-cli-flow-recovery.sh` PASS before T25 commit; each engine call bounded below outer watchdog with cleanup margin and durable phase identity |
...
| Registered suite | `FF_FULL=1 FFHC_HEARTBEAT_SECS=30 bash hooks/tests/run-tests.sh` once on final T25 source |
...
- T11-T20, T24 and T25 each have one exact SHA and passed focused checks. T24 may commit its single fixture file after focused proof; full wrapper is a distinct T25 blocker.
- T25 requires full registered recovery wrapper PASS; targeted U17/U18 rc0 HEALTHY alone cannot close the nondeterministic stall. Keep failed attempt evidence and reproduce/explain before PASS.
- Dependency tail: T20 -> T24 -> T25 -> T21 -> T22 -> T23. T21 resumes only after T25; it replaces the provisional T10/T21-blocked report and does not commit source.
