...
| S1 | Shared minimal-PATH fixture + mutation discriminator | Current absent-interpreter rows remain green after deleting the missing-interpreter exit; two host-directory masks and an arbitrary cap duplicate the wrong layer | Blocked only by S0; its shipped harness is also subject to S15's structured comparison contract | Full | M | `hooks/git/pre-commit:93-96`; `hooks/tests/test-bootstrap-exception.sh:255-300`; `hooks/tests/test-git-hooks-smoke.sh:78-130`; `bd47aed`; `800bf36`; `6ffd8da` |
| S15 | Strengthen mutation-comparison contract | Row-only `PASS|FAIL` reduction cannot compare diagnostics, artifacts, timing class, or side effects | Applies to the harness shipped in S1 and every later discriminator; each mutation predeclares its expected observable and compares structured rc + diagnostics + artifact/side-effect state | Full | S | `docs/specs/pre-commit-trusted-tool-contract/spec.md:95`; `docs/specs/pre-commit-trusted-tool-contract/verification-gate.md:31-44` |
| S2a | Lock trusted Git/Python verdict contract | The trusted-tool definition is unmade: executable presence, version, invocation success, and positive-verdict evidence have different failure surfaces | DESIGN-first; independent of S1; blocks S2b and S3; S2c remains reopened design only | Full | S | `hooks/git/pre-commit:20,87,213,316,503,678` |
...
| S2c | Reopen positive Python verdict design | B1 catches only a naive zero-exit stub; a same-principal PATH attacker can fabricate the visible verdict while the design adds four seams, temp lifecycle, concurrency, and helper-bootstrap risk | **DESIGN/REOPENED** - operator chooses drop vs a much smaller accidental-completion-fault protocol explicitly not treated as security; no implementation task exists | Full | M | `docs/specs/pre-commit-trusted-tool-contract/decisions.md`; `hooks/git/pre-commit:316-320,678-682` |
...
| 2 | S2b, S3, S15; S2c design only | S2b follows S2a and supplies its own missing-Git seam; S15 upgrades the S1 and future mutation contract; S2c stays **DESIGN/REOPENED** with no implementation; S3 follows locked S2a and does not await S2b/S2c implementation |
