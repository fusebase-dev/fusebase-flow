...
| S2c | ~~Reopen positive Python verdict design~~ | **CLOSED — NOT BUILT** (operator, 2026-08-11). A PATH-controlling same-principal attacker can read the nonce and fabricate the verdict, so the mechanism cannot deliver the guarantee its name implies; reviving it requires a changed threat model, not a new design | No work exists; no successor slice | — | — | `docs/specs/pre-commit-trusted-tool-contract/decisions.md` |
...
| 2 | S2b, S3, S15 (S2c is CLOSED — no work) | S2b follows S2a and supplies its own missing-Git seam; S15 upgrades the S1 and future mutation contract; S2c stays **DESIGN/REOPENED** with no implementation; S3 follows locked S2a and does not await S2b/S2c implementation |
