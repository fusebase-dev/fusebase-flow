...
1. Final pre-deploy checklist:
   - [ ] Deploy scope enumerated so the Deploy session can author the right approval artifact(s) on the DP.6 phrase: `production_deploy` + any `database_migration` / `auth_or_permission_change` / `protected_path_edit` the deploy needs. The artifacts need NOT exist at handoff time — the Deploy session authors them on the operator's `approve deploy now` phrase (for all tickets; the operator never runs approval commands). <!-- prevents: unauthorized-deploy (catastrophic-low-frequency) — taxonomy: policies/ratchet-governance.yml -->
...
1. Read deploy handoff. Verify the required approval artifact(s) exist — or note that you'll author them at step 4 on the operator's DP.6 phrase (all tickets; present the full scope in chat first for sensitive actions).
2. Self-attest: "Operating as Deploy phase under Fusebase Flow v4.17.1. I will follow FR-01 through FR-27, including FR-05 (gate fulfilled), FR-06 (reversible by default), FR-07 (worker-undisturbed), FR-12 (approval-gated), FR-14 (single docs commit on deploy), FR-19 (chat-text questions). I will apply the role-discipline skill section for Deploy phase (DP.1..DP.12)."
...
4. **Operator confirm (DP.6 + FR-19).** Ask the operator in chat text to reply `approve deploy now` — a **forgiving** match (any case / spacing / hyphenation; legacy `APPROVE-DEPLOY-NOW` still passes), but it must be that deliberate three-word phrase, not a bare "yes" / "ok" / "go". The operator types **ONLY** the phrase — never ask them to compose a sentence, prepend the ticket / scope, or answer other questions in the same breath; the scope is your one-line summary shown just above the ask. Do not use popup / clickable menu tools. If the response isn't the phrase, ABORT and surface it; the operator re-issues by re-running this workflow in a fresh session. <!-- prevents: unattended-prod-cutover (catastrophic-low-frequency) — taxonomy: policies/ratchet-governance.yml -->
   **Author the approvals on the DP.6 phrase (all tickets).** The operator's typed `approve deploy now` (any case/spacing) authorizes you to author every required approval artifact on their behalf — run `bash hooks/local/approve-local.sh <action> <slug> 'approve deploy now' --command '<exact command>'` for EACH action the deploy needs (`production_deploy` + `database_migration` / `auth_or_permission_change` / `protected_path_edit` …), then proceed. Do NOT make the operator run these commands. The artifacts still exist on disk (FR-12 + hook semantics unchanged); only the authoring party changes — the phrase is the authorization, the JSON is bookkeeping (the gate checks schema 3, parsed expiry, body/filename `action` agreement and the MANDATORY command/repo/profile binding — and for a gated `git push` every ref update it performs, so mint that one AFTER the last commit you intend to push and run the push as its own plain command; `approved_by` / `ticket` are audit metadata, never authenticated authority — `policies/approval-policy.yml` § trust model, decision K3). BEFORE the phrase, present the full scope in chat as YOUR one-line summary so it is informed consent for every action — the operator reads it and types only the phrase; it is never text for them to compose. `dp1_waiver: eligible` = reversible fast-path; `excluded`/sensitive classes (migration / security / protected-path) require presenting the full scope before the phrase, but you — not the operator — still author the artifacts. Authoring WITHOUT the phrase is self-approval and is forbidden.
