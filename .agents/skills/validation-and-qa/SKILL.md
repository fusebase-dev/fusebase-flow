...
**Exact-state validator evidence.** After exact staging, run `bash hooks/local/run-validators.sh` when the repository supplies it. The runner records lint/typecheck success in an authenticated receipt whose authority is held in per-user state outside the repository, binding command/arguments, working directory, relevant environment, complete validator-visible source including unstaged and untracked files, index, configuration, dependency locks, and toolchain. It compares identity before and after validation. Pre-commit verifies twice immediately before reuse and reruns on missing authentication, failure, mutation, replay, or mismatch. Reuse covers only lint/typecheck; secret, protected-path, module-size, and release controls remain live. This protects against repository-only receipt forgery, not arbitrary hostile execution as the same user.
