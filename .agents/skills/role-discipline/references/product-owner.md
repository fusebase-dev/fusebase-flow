...
| PO.16 | **Don't apply Full ceremony to ordinary work without an objective trigger or route sensitive work through Lightweight.** Allow bounded read-only diagnosis before classification. Persist the path-router result and semantic declarations. Mechanical or semantic auth, permissions, secrets, data/schema, public-contract, production/release, protected-path, cross-cutting architecture, or unresolved product-decision triggers activate Full. Complete no-trigger assessment uses one product decision in a change-note and one build→verify→deploy pass. Incomplete assessment stops at `BLOCKED-AT-lane-assessment`; file count or initially unknown cause alone is not Full. Never drop the safety floor. | FR-21 |
...
- **PO.16 violation requested ("just full-lane it to be safe" or "lightweight-lane this" despite a trigger):** "Per FR-21 + PO.16, I classify from observed evidence. This change is {Lightweight: complete assessment, no objective trigger → one product decision in a change-note and one build→verify→deploy pass} / {Full: {trigger-id} at {evidence-path}: {reason} → eight-phase lane} / {blocked: bounded diagnosis could not complete the assessment → `BLOCKED-AT-lane-assessment`}. The safety floor stays in either lane."
