...
The verdict-affecting operations (preflight, conflict reporter, the upstream `git fetch`) are bounded so a network-impaired or large-repo host can't make the read-only diagnostic appear to hang. A timed-out/skipped **critical** check ⇒ `PARTIAL_UNVERIFIED` (never a false `HEALTHY`). **(v4.2.0+)** the hook-layer integrity critical is a fast single-pass manifest hash-compare (`audit/hook-layer-manifest.json`; seconds, OS-independent), so a full **HEALTHY** verdict is now reachable on Windows/Git-Bash **without** `--fast` — a mismatch names the exact drifted file (`FLOW_LAYER_DRIFT`, exit 1) and also catches local tampering. The upstream comparison is **optional** — its fetch timing out is a "upstream not verified" note only and never forces exit 4.
...
| `--fast` | Skip the hook-layer-integrity critical (and upstream); keeps preflight + inventory + conflict reporter. **Explicitly partial.** (v4.2.0+: **no longer needed on Windows** — the integrity critical is a fast manifest verify, so a full run is HEALTHY in seconds; kept for back-compat.) | **4, never 0** (prints "fast mode — not a full health verdict") |
| `--skip-hook-tests` | Alias for `--fast`. | **4, never 0** (same as `--fast`) |
| `--run-hook-tests` | OPTIONAL deep diagnostic (never gates the verdict). **Platform-adaptive** (v4.2.0+): POSIX runs the FULL `run-tests.sh`; MSYS/Git-Bash runs the FAST subset (single-process fixtures + git-smoke + hook-manifest, < 120s). A real FAIL/crash ⇒ `BROKEN`; a timeout is a note only. | 0 if HEALTHY (the deep run never forces exit 4) |
| `--run-hook-tests-full` | Force the FULL `run-tests.sh` even on MSYS (also `FFHC_RUN_HOOK_TESTS_FULL=1`). | as above |
