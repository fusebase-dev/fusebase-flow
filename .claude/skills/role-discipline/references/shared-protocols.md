...
| Any other approval-gated action (other protected-path categories, migration, auth/permission, session-key use) | OKs that specific action in chat | authors the artifact (`approve-local.sh <action> <slug> --command '<exact command>'` — e.g. `protected_path_edit` + paths), runs the command |
