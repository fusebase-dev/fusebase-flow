...
3. If `docs/tmp/handoff.md` exists, archive it to `docs/tmp/handoff/archive/<YYYY-MM-DD-HHMM>-handoff.md` first (paper trail). Then write `docs/tmp/handoff.md` fresh (FR-18) using `templates/handoff.md` — all 16 sections, `Unknown` not guesses, `None` for empty, pointers to canonical spec/decisions/tasks instead of reprinting (FR-23). Exactly one concrete Next Step.
