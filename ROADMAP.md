...
|---|---|---|
| **Session-floor + report-budget remediation** (in flight, v4.6.0) | Consumer `/token-waste-audit` over 5 sessions measured a **17.8–19.4k-token role-aware boot floor** paid before any work starts, plus five compounding defects. Eight slices: delegated chat-return budget · supersede-vs-write-primitive clarification · exact-body anti-reread rule with a corrected per-surface auto-load matrix · Amendment-log extraction to `FLOW_RULES_HISTORY.md` (behind a compatibility stub) · boot-core compression of both mandatory skills and the rules file to ≤36k chars with zero rule loss · labeled auto-classification in `token-waste-audit.py` · zero-trust delegated-session liveness recipe · release bookkeeping. | [`docs/specs/token-floor-remediation/`](docs/specs/token-floor-remediation/spec.md) |
