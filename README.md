...
[![Version](https://img.shields.io/badge/version-4.7.1-blue.svg)](VERSION)
...
| **Upgrade Flow content** (skills/rules/agents) to latest upstream | **From an install ≤ 4.6.1 the only supported route is `bash hooks/local/bootstrap-upgrade.sh -- --auto-yes`** — your installed engine predates the classifier and cannot deliver 4.7.0 correctly (why: [Upgrading an installed overlay](#upgrading-an-installed-overlay-v360)). Already on 4.7.0+: `bash hooks/local/upgrade.sh` (refresh `.fusebase-flow-source/` first; `--dry-run` to preview) |
...
Recovery (`post-fusebase-update.sh`) restores **only** Flow-owned assets and shared Flow additions — Flow skill/agent mirrors, `AGENTS.md`/`CLAUDE.md` overlay blocks, the `fusebase-flow-health-check` skill, and `.claude/commands/fusebase-health.md`. It never touches `.claude/hooks/**`, CLI provider skills, MCP config, `fusebase.json`, `skills-lock.json`, or active `.codex/config.toml`. **Hook wiring is opt-in, and the opt-in is remembered** (opt-in since v3.6.0; automatic restoration since v4.15.0): recovery touches `.claude/settings.json` only where an earlier `--wire-hooks` recorded an ENABLED wiring intent for the `claude_settings` surface (`state/audit/flow-hook-wiring-intent.json`). Such a tree gets its Flow lifecycle events restored automatically and the run reports which events it added. With no marker, recovery leaves the file alone — pass `--wire-hooks` to opt in, or `--forget-hook-wiring` to record an opt-out. The CLI's own Stop hooks are preserved either way.
...
- **`bash hooks/local/upgrade.sh`** — the in-place **content** upgrade. Use this when a newer Flow ships new/changed skills, rules, agents, workflows, policies, templates, **or hooks**. It refreshes the canonical content (incl. the `hooks/` layer — so the hook handlers and the upgrade tooling itself stay current) from `.fusebase-flow-source/`, re-mirrors it, syncs the derived attestation strings (version + FR-range + skill count), refreshes drifted AGENTS.md/CLAUDE.md overlay blocks **carrying your `FLOW:PRESERVE` region forward** (it never clobbers your `### Project-specific values`), then bumps `VERSION` **last**. `--dry-run` previews the plan; backups are written with a `.pre-upgrade-<ts>` suffix and are removed — once you have validated the upgrade — with `bash hooks/local/cleanup-flow-backups.sh --all` (or by naming exact targets). That tool's destructive authority is exact managed-stem membership plus an exact `.pre-upgrade-<YYYYMMDDTHHMMSSZ>` suffix, so it refuses absolute paths, `..`, globs, symlinks, non-members and anything resolving outside the repo root. It does **not** copy framework dev-docs into your `docs/` — pass `--with-framework-docs` to stage them under `docs/_fusebase-flow/`.
...
> ⚠️ **Upgrading FROM ≤3.20.0: prefer the bootstrap path.** Engines before v3.20.1 overwrite their own running script while refreshing `hooks/` and can abort mid-upgrade with a `syntax error` (offset-dependent; deterministic on some hops). Two safe options: run `bash hooks/local/bootstrap-upgrade.sh -- --auto-yes` (stages the new engine *first*, so the self-overwrite is byte-identical and harmless — works on any 3.8.0+ install, not just pre-3.6.0), or simply **re-run `upgrade.sh` after the abort** — the refreshed engine completes the remaining steps idempotently. v3.20.1+ engines parse the whole file before executing (`main()` wrapper) and are immune.

> ⚠️ **Upgrading FROM a pre-v3.25 install: use the bootstrap path.** Run `bash hooks/local/bootstrap-upgrade.sh -- --auto-yes` for the v3.25.x hop. v3.25.0 added a baseline merge-preserve step (it keeps your project's own over-ceiling `policies/module-size-baseline.txt` rows instead of letting the wholesale `policies/` refresh clobber them), but **that merge logic ships inside the version you're upgrading TO** — your *installed* old `upgrade.sh` has no merge code, and even running a freshly-copied `upgrade.sh` directly fails if its sourced lib (`hooks/local/lib/`) hasn't been staged first. The bootstrap path stages the new engine **and** `hooks/local/lib/` before handoff, so the merge actually runs on the adoption hop. If a baseline was clobbered (you'll see a loud `module-size baseline was NOT merge-preserved` warning, or `check-module-size --all` starts failing), it is recoverable: `cp policies/module-size-baseline.txt.pre-upgrade-<ts> policies/module-size-baseline.txt` (the `.pre-upgrade-<ts>` backup), then re-run via the bootstrap path.

> ⚠️ **Upgrading FROM ≤4.6.1 to 4.7.0+: the bootstrap path is the only supported route — running your installed `upgrade.sh` is unsupported.** Run `bash hooks/local/bootstrap-upgrade.sh -- --auto-yes`.
>
> **This is not theoretical.** A prerelease consumer ran their installed 4.5 engine instead, and the two HIGH bugs they then filed — a new top-level file never installed, and a hand-edited `policies/` entry silently reverted — were both **artifacts of the stale engine**, not of 4.7.0: 4.7.0 drives the managed set from a manifest and classifies every managed path per file. Full write-up: `docs/release-notes/v4.7.0.md` § "three reported findings need no code". 4.7.0 replaces the whole-directory `cp -R` refresh with a **three-way classifier**: it compares what upstream shipped you last time (the *base*, `audit/managed-content-manifest.json`) against your tree and the incoming tree, so it can tell **your** edits from **upstream's** and stops silently overwriting local hardening. Two reasons the bootstrap path is required rather than merely preferred: (1) the classifier ships *inside* the version you are upgrading to, and the old engine would copy upstream over `hooks/**` — including the new engine — before any classification ran; (2) an install from ≤4.6.1 has **no base manifest**, and without one every managed path classifies `unknown-base`, which K9 *preserves* — the upgrade would report success while installing nothing. The bootstrap path fixes both: it stages the new engine first, then **synthesizes the base** from the upstream tag matching your installed `VERSION` (e.g. `v4.6.1`), which is byte-identical to what your last install/upgrade actually wrote. If that tag cannot be resolved (a forked or unreleased `VERSION`), the run continues with `unknown-base` — everything is **preserved and reported**, never overwritten, but little is refreshed; record a base yourself with `bash hooks/local/stamp-managed-content-manifest.sh` and re-run. Paths where **both** you and upstream changed the same file are `changed-by-both`: `--auto-yes` **aborts** on them without writing anything, names each one, and tells you the resume command. `bash hooks/local/preflight.sh` warns when the base is missing or stale.

> ⚠️ **`.fusebase-flow-source/` + ESLint (v3.8.6+).** The staging clone holds CLI-owned **CommonJS** hooks, and ESLint **flat config does not read `.gitignore`** — so if your `fusebase deploy` runs lint, it lints the clone and fails (`@typescript-eslint/no-require-imports`) even with zero app errors. The clone is **transient**: remove it after an upgrade through the sanctioned entry point (`bash hooks/local/cleanup-flow-backups.sh .fusebase-flow-source` — re-created next upgrade; a raw `rm -rf` is denied by `policies/command-policy.yml` under FR-06), or add `".fusebase-flow-source/**"` to your `eslint.config` `ignores` next to `".claude/**"`. One-shot helper: `bash hooks/local/eslint-ignore-flow-paths.sh` (idempotent; backs up your config).
...
Hook wiring is a deliberately separate, opt-in step (`post-fusebase-update.sh --wire-hooks`). `upgrade.sh` never edits `.claude/settings.json` itself, but since v4.15.0 the recovery step it runs restores the Flow lifecycle events on a tree that already recorded an ENABLED wiring intent — the upgrade summary reports that outcome, naming the events it added.
...
├── .agents/skills/                 ← Codex skill surface (34 Flow mirrors + 22 CLI provider skills)
├── .claude/skills/                 ← Claude Code skill surface (34 Flow mirrors + 22 CLI provider skills)
...
- [`docs/rail-mapping.md`](docs/rail-mapping.md) — FR-01..FR-27 → enforcement surfaces
