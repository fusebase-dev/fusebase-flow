...
# (R2 — the live tree is never mutated). 6 scenarios.
#
...
# 1. stamp byte-idempotence (D1 — no "modulo generated_at" allowance).
...
# 3. tampered covered file => rc 1 naming the file.
...
# 4. Scan A — extra hooks/shared/*.py not in the manifest => rc 1.
...
# 5. Scan B — sitecustomize.py nested under hooks/tests/ => rc 1, python-startup-file.
...
# 6. corrupt self-hash => rc 2 BROKEN; then absent manifest => rc 4 ABSENT (SF8).
"$python_bin" - "$MAN" <<'PY'
