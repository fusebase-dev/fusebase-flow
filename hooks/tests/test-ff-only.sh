...
if [ "$release_count" -eq 36 ] && [ "$release_core" -eq 1 ] && [ "$release_nonmember" -eq 1 ]; then
  ok "release-profile-is-explicit-36-tag-allowlist"
else
  bad "release-profile-is-explicit-36-tag-allowlist" "run_count=$release_count or boundary tags differ"
fi
