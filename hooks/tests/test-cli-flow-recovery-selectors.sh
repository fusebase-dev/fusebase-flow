...
run_case t34 env TMPDIR="$WORK" FFCF_SELECTOR_TIMEOUT_SECS=180 bash "$WRAPPER" --only t34
if [ "$CASE_RC" -eq 0 ] \
  && grep -q '^SCOPED PASS: cli-flow-recovery T34:' "$WORK/t34.out" \
  && ! grep -q 'U14:' "$WORK/t34.out"; then
  ok "selected-t34-runs-real-group-only"
else
  bad "selected-t34-runs-real-group-only" "rc=$CASE_RC"
fi
if ! grep -q '^PASS: cli-flow-recovery ' "$WORK/t34.out" \
  && grep -q '^SCOPED: cli-flow-recovery group=t34; not full-suite evidence$' "$WORK/t34.out" \
  && grep -qE '^\[cli-flow-recovery\] SCOPED group=t34 START elapsed=0s timeout=180s$' "$WORK/t34.err" \
  && grep -qE '^\[cli-flow-recovery\] SCOPED group=t34 END elapsed=[0-9]+s rc=0$' "$WORK/t34.err"; then
