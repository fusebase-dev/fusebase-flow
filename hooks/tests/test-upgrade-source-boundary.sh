...
#
# Three responsibility seams around this file: test-upgrade-conflict-classification.sh owns K9
# classification, test-upgrade-repair-managed.sh owns AC3's deliberate byte repair,
# test-upgrade-preboundary-consumed-tree.sh owns the pre-boundary route (the tree we PROVED vs
# the tree the engine CONSUMES — B4/B5/B6), and this one owns WHICH BYTES enter the consumer —
# and, per the source-shape matrix below, which source shapes are admitted at all. Fixture builders are shared from lib/upgrade-fixtures.sh. The three
# byte models
