...
3. If `docs/tmp/handoff.md` exists, archive it to `docs/tmp/handoff/archive/<YYYY-MM-DD-HHMM>-handoff.md` first (paper trail; archiving a live `Mode: run-ledger` file here is the sanctioned supersede). Then supersede `docs/tmp/handoff.md` (FR-18) using `templates/handoff.md` — all 16 sections, header stamped `Mode: restart`, `Unknown` not guesses, `None` for empty, pointers to canonical spec/decisions/tasks instead of reprinting (FR-23). Exactly one concrete Next Step. **Write primitive:** supersede replaces stale *semantics*, not the file — a `/handoff` restart snapshot over a live ledger is a full `Write` (mode transition); a run-ledger update where most sections are unchanged is a targeted `Edit`.
