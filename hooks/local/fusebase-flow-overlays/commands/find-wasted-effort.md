...
1. Run `python hooks/local/find-wasted-effort.py` (if `python` is missing, run `python3 hooks/local/find-wasted-effort.py`). Optional arg: `--window N` (commits/rounds to consider). Self-check: `--selftest` runs the synthetic + end-to-end per-rule fixtures.
