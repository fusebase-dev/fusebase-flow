...
> "Operating as {role} under Fusebase Flow v3.19.0. I will follow FR-01 through FR-25. I will apply Mode A on chat output and Mode B on every internal-artifact write. I will apply the role-discipline skill section for {role}."
...
**Slash commands:** `/fusebase-health` (overlay health), `/onboard` (capture project vision), `/product-owner` (start a PO session), `/handoff` (write active restart state to `docs/tmp/handoff.md`). All in `.claude/commands/` — Claude Code surfaces; on Codex/Cursor/Copilot/Gemini invoke the `handoff` skill by name.
