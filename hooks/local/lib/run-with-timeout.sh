...
# Windows Job Object OUTER FENCE (opt-in, DEFAULT OFF) — extracted to lib/job-fence.sh per FR-25.
# Sourced AFTER run_with_timeout/ffhc_is_msys/ffhc_timed_out exist, because it uses them. A tree
# that ships this file WITHOUT job-fence.sh (test fixtures do) stays correct: the stub below makes
# the capability answer "unavailable", and the knob-first gate means the default path never asks.
_FFHC_JF="$(dirname "${BASH_SOURCE[0]:-$0}")/job-fence.sh"
# shellcheck source=job-fence.sh
if [ -r "$_FFHC_JF" ]; then . "$_FFHC_JF"; fi   # `if`, not `&&`: unambiguous under set -e
command -v ffhc_job_available >/dev/null 2>&1 || ffhc_job_available() { return 1; }
