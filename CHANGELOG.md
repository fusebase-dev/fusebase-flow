...
(`5.1`, `5.2`, `6.0`; tags `v5.1`, …). The jump from 4.17.1 consolidates the 4.17.x
approval-binding command-authorization cutover into a clean major line. **Drop-in from 4.17.1 —
no runtime behavior change beyond 4.16.x–4.17.1.** Historical three-part version strings stay
recognised.
...
`preboundary-consumed` promoted into the essential release profile. Internal: gate-bounds cost
attributed with measured timings (no wall changed), a git-context oracle corrected, a convergence
discriminator added, and a proposed FR-28 drift-checkpoint rule reviewed and declined. See
`docs/release-notes/v5.1.md`.
...
tag was left in place rather than moved; this release supersedes it. Everything v4.9.0 intended to
ship is here, with the false state corrected.

### Fixed — AI-consumed artifacts described a repository that does not exist

This framework's product is documents that agents load and act on, so a document asserting a false
repository state is a defect in the product, not a typo.

- `docs/specs/consumer-escalation-v480/spec.md` listed the reverted `INSTALLED_FROM` work as
  **Landed** and documented its schema as current behaviour. The feature was reverted; the file does
  not exist. It is now marked REVERTED — NOT SHIPPED, with its contract and acceptance criteria kept
  as historical analysis so the reasoning survives without reading as delivered behaviour.
- `docs/tmp/handoff.md` named an obsolete branch, HEAD and merge state. Superseded in place.
- `docs/backlog/release-gate-flaky-job-probe/README.md` stated a hypothesis as an identified root
  cause. It is now separated into what is established (the watchdog return code is preserved and
  then discarded by the probe), what is excluded, and what remains open (a slow interpreter start is
  plausible but unproven). The prescribed next step is instrumentation, not the retry it previously
  recommended.

An adopter's agent could otherwise have rebuilt reverted work, or treated shipped fixes as unmerged,
on the authority of those files.

### Fixed — a released tree cannot fingerprint itself

`docs/release-fingerprints.md` promised identification without upgrading, but by construction could
not contain the row for the release it ships in: adding the row changes the digest it describes. The
limit is now stated, the v4.9.0 tree's row is published, and `PUBLISHING.md` gains the post-tag step
whose absence created the gap. Rows are generated by
`hooks/local/print-release-fingerprints.sh`, never transcribed.

Everything under [4.9.0] below is included in this release.

## [4.9.0] — 2026-08-12 — TAGGED, NEVER PUBLISHED
...
Field escalation (High): `upgrade.sh` / `bootstrap-upgrade.sh` / `post-fusebase-update.sh` leave untracked backup copies of the hook layer in the worktree (`hooks.pre-upgrade-<ts>/`, `policies.pre-upgrade-<ts>/`, `*.pre-bootstrap-<ts>`, `AGENTS.md.pre-refresh-<ts>`, …). The `hooks.pre-upgrade-<ts>/` snapshot carries the OLD secret-scan test fixtures and `policies.pre-upgrade-<ts>/` carries `secret-patterns.yml` — files that deliberately contain dummy `ghp_`/`sk-ant-`/cookie literals (the `.pre-bootstrap`/`.pre-refresh` backups don't, but are git-excluded all the same). The scanner path-excluded the LIVE fixture/policy locations but not their renamed backup twins, so a wholesale `git add -A` — notably FuseBase CLI's `fusebase update` "create pre-update checkpoint? Yes" — hard-BLOCKED on Flow's own backups with misleading "rotate the credential" advice, then left the blobs staged (`AD`) so every later commit kept blocking. Operator symptom: "fusebase update doesn't work."

- **Scanner excludes ONLY the fixture/policy backup twins** (not namespace-wide). `hooks/shared/staged_secret_scan.py` adds root-anchored `:(exclude,glob)hooks.pre-upgrade-<ts>/tests/fixtures/**` + `policies.pre-upgrade-<ts>/secret-patterns*.yml`, where `<ts>` is the EXACT `[0-9]{8}T[0-9]{6}Z` stamp. Deliberately NOT a broad `*.pre-*`, NOR a loose `*T*Z` (which matches literal `TZ` at any depth) — both are scanner bypasses (Codex-xHigh findings). A backup of any other file, a `TZ`/non-root/wrong-prefix look-alike, and any untimestamped name all stay scanned. These twins are copies of the SAME files the live D-A1 excludes already exempt (`hooks/tests/fixtures/`, `secret-patterns.yml`), so this extends that existing exemption to their upgrade-backup copies — not a new content category; pathspec matches by name only, so the exact `[0-9]{8}T[0-9]{6}Z` stamp + root-anchoring keep the exempted set as narrow as the live D-A1 set. The installed `hooks/git/pre-commit` block message was updated to match (not only `staged_secret_scan.py:main()`).
- **Backups git-excluded at creation.** `upgrade.sh`, `bootstrap-upgrade.sh`, and `post-fusebase-update.sh` append EXACT-timestamp patterns (`*.pre-upgrade-[0-9]{8}T[0-9]{6}Z` …) to `.git/info/exclude` (via `git rev-parse --git-path`, worktree-correct; ensures a trailing newline first so an unterminated line can't swallow the rule) before writing backups, so `git add -A` / the CLI checkpoint never stages them (also prevents the `AD`-staged trap). Per-pattern idempotent, `set -e`-safe, and honest on failure (WARN + conditioned note). Keep backups until validated (only previously-committed content is recoverable from git history).
- **Accurate remedy.** The BLOCK message no longer claims a `.pre-*` hit is "not a real secret" (only the exact fixture/policy twins are skipped) — treat a remaining hit as **potentially real, inspect it**, rotate if real, and separately unstage the exact Flow backup path a pre-fix `git add -A` left staged; `upgrade.sh` conditions its "git-excluded" note on the actual `.git/info/exclude` write and adds "answer No to the CLI checkpoint prompt mid-ticket" for the separate FR-25-on-WIP-growth interaction.
- Test + problem-catalog entry `upgrade-backups-trip-secret-scan`. Reviewed by Codex gpt-5.6-sol (High).
